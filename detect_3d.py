...
def triangulate_point(point1, point2):
    ...
    print(f"Undistorted points - Camera 1: {point1_undistorted}")
    print(f"Undistorted points - Camera 2: {point2_undistorted}")
...
def calculate_reprojection_error(point_3d, point1, point2, mtx1, dist1, mtx2, dist2):
    ...
    error1 = np.linalg.norm(point1 - reprojected_point1.squeeze())
    error2 = np.linalg.norm(point2 - reprojected_point2.squeeze())
...
# Specify the target AprilTag ID
target_id = 0

print(f"Looking for AprilTag with ID: {target_id}")
...
while True:
    ...
    # Detect specific AprilTag in both frames
    point1, id1 = detect_specific_apriltag(frame1, target_id)
    point2, id2 = detect_specific_apriltag(frame2, target_id)
    
    if point1 is not None and point2 is not None:
        point_3d = triangulate_point(point1, point2)
        print(f"Target AprilTag detected!")
