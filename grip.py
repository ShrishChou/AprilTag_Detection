        ...
        try:
            ...
            for i in range(int(10)):
                ...
                if not self._check_code(code, 'set_position'):
                    return
                code = self._arm.set_position(*[-79.8, 412.1, 170.0, 180.0, 0.0, 76.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=False)
                if not self._check_code(code, 'set_position'):
                    return
                # Pick object
                ...
                if not self._check_code(code, 'set_gripper_position'):
                    return
                # Set TCP payload as the xArm Gripper and Object
                ...
                if not self._check_code(code, 'set_position'):
                    return
                code = self._arm.set_position(*[-350.0, 412.1, 400.0, 180.0, 0.0, 76.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=False)
                if not self._check_code(code, 'set_position'):
                    return
                code = self._arm.set_position(*[-350.0, 412.1, 170.0, 180.0, 0.0, 76.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=False)
                if not self._check_code(code, 'set_position'):
                    return
                # Drop object
                code = self._arm.set_gripper_position(800, wait=True, speed=5000, auto_enable=True)
